// Project Edit Interface JavaScript
...
function initializeEventListeners() {
    ...
    document.getElementById('load-images-btn').addEventListener('click', loadNewImages);
    ...
    try {
        ...
        if (data.projects && data.projects.length > 0) {
            ...
            data.projects.forEach(project => {
                ...
                option.textContent = `${project.name} (${project.collaborator})`;
                select.appendChild(option);
            });
        }
    } catch (error) {
        showMessage('Failed to load projects list', 'error');
    }
}
...
async function handleProjectSelect(e) {
    ...
    try {
        showMessage('Loading project...', 'info');
        
        const response = await fetch(`/api/load-project/${slug}`);
        const data = await response.json();
        ...
        // Populate form
        currentSlug = slug;
        document.getElementById('name').value = data.name.replace(`, ${data.year}`, '');
        document.getElementById('year').value = data.year;
        document.getElementById('collaborator').value = data.collaborator;
        document.getElementById('official_site').value = data.official_site || '';
        document.getElementById('role').value = data.role || '';
        document.getElementById('description').value = data.description || '';
        document.getElementById('acknowledgment').value = data.acknowledgment || '';
        
        // Load existing images
        originalImages = data.images || [];
        loadedImages = [...originalImages];
        
        // Convert existing images to use base64 for display
        await renderExistingImages();
...
async function renderExistingImages() {
    const container = document.getElementById('images-container');
    
    if (loadedImages.length === 0) {
        container.innerHTML = '<p class="placeholder-text">No images in this project</p>';
        return;
    }
    
    // Load base64 data for each existing image
...
async function loadNewImages() {
    ...
    try {
        ...
        await renderExistingImages();
        updatePreview();
        showMessage(`Loaded ${loadedImages.length} new images (will replace existing on save)`, 'success');
    } catch (error) {
        showMessage(error.message, 'error');
    }
}
...
async function handleSubmit(e) {
    ...
    try {
        ...
        const imagesChanged = JSON.stringify(loadedImages) !== JSON.stringify(originalImages);
        ...
        const response = await fetch(`/api/update-project/${currentSlug}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        showMessage('Project updated successfully! Redirecting...', 'success');
...
function disableForm() {
    ...
    document.getElementById('preview-title').textContent = 'Select a project to edit';
    document.getElementById('preview-collaborator').textContent = 'Collaborator';
    document.getElementById('preview-year').textContent = '2024';
    document.getElementById('preview-description').innerHTML = '<p>Description will appear here...</p>';
    document.getElementById('preview-images').innerHTML = '';
    document.getElementById('preview-acknowledgment').innerHTML = '';
    
    // Remove dynamic rows
    const propertiesTable = document.getElementById('preview-properties');
    propertiesTable.querySelectorAll('.official-site-row, .role-row').forEach(row => row.remove());
}
